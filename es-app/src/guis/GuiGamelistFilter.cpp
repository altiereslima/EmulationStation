...
GuiGamelistFilter::GuiGamelistFilter(Window* window, SystemData* system) : GuiComponent(window), mMenu(window, "FILTRAR LISTA DE JOGO POR"), mSystem(system)
{
	initializeMenu();
}
...
void GuiGamelistFilter::initializeMenu()
{
	...
	// show filtered menu
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "RESTAURAR TODOS OS FILTROS", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	mMenu.addButton("VOLTAR", "voltar", std::bind(&GuiGamelistFilter::applyFilters, this));
