	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "EDITAR INFORMAÇÕES", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(!scraperParams.system->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "OBTER", "obter", std::bind(&GuiMetaDataEd::fetch, this)));

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SALVAR", "salvar", [&] { save(); delete this; }));
	...
	if(mDeleteFunc)
	{
		...
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "ISTO IRÁ EXCLUIR UM ARQUIVO!\nVOCÊ TEM CERTEZA?", "SIM", deleteFileAndSelf, "NÃO", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "EXCLUIR", "excluir", deleteBtnFunc));
	}
