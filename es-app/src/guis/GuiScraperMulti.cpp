	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "OBTENDO DADOS", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(approveResults)
	{
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "INSERIR", "pesquisar", [&] {
			mSearchComp->openInputScreen(mSearchQueue.front());
			mGrid.resetCursor();
		}));

		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "PULAR", "pular", [&] {
			skip();
			mGrid.resetCursor();
		}));
	}

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "PARAR", "parar (progresso salvo)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "JOGO " << (mCurrentGame + 1) << " DE " << mTotalGames << " - " << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NÃO FORAM OBTIDOS DADOS DE NENHUM JOGO.";
	}else{
		ss << mTotalSuccessful << " JOGO" << ((mTotalSuccessful > 1) ? "S" : "") << " COM DADOS ENCONTRADOS!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " JOGO" << ((mTotalSkipped > 1) ? "S" : "") << " PULADO(S).";
	}
